...
**VPN egress proxy** (`media` namespace, Gluetun) gives any workload anonymous, non-home-IP egress (HTTP CONNECT + SOCKS5) — used by DNS upstream, scrapers, indexers, and threat-intel feeds. A watchdog CronJob restarts it if it wedges.
...
| **media-mcp** | media automation stack | One bearer-gated endpoint over the media stack for cross-tool workflows (look up → add → search → scan → confirm) |
...
- **media/** — Immich, Audiobookshelf, Kavita, Calibre-Web-Automated, Dispatcharr, media automation, MinIO / VPN egress
