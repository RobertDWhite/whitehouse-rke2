...
export default function Reconciliation() {
  ...
  const load = () => api.reconciliation().then(setData).catch(() => setData(null))
  useEffect(() => { load() }, [])
  const resolve = async (id, status) => {
    await api.reconciliationResolve(id, status).catch(() => {})
    load()
  }
    ...
    <>
      ...
      <div className="cards">
        ...
        {Object.entries(data.by_status || {}).map(([k, v]) => (
          <div className="card" key={`s-${k}`}><div className="label">{k}</div><div className="big num">{v}</div></div>
        ))}
      </div>
      <h2>Open Issues</h2>
      <div className="panel" style={{ padding: 0, overflowX: 'auto' }}>
        <table>
          <thead><tr><th>Kind</th><th className="right">Severity</th><th className="right">Confidence</th><th>Source</th><th>Detail</th><th>Seen</th><th></th></tr></thead>
          <tbody>
            {(data.items || []).map((r) => (
              <tr key={r.id}>
                <td><span className={`tag ${r.severity >= 3 ? 'sell' : 'exch'}`}>{LABELS[r.kind] || r.kind}</span></td>
                <td className="right num">{r.severity}</td>
                <td className="right num">{r.confidence == null ? '—' : `${Math.round(r.confidence * 100)}%`}</td>
                ...
                <td className="nowrap">
                  ...
                  <button className="btn-sm" onClick={() => resolve(r.id, 'ignored')}>Ignore</button>
                </td>
              </tr>
            ))}
          </tbody>
