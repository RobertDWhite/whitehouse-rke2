...
for monica.white.fm, cloning the flow/settings from an existing proxy
...
Optional env:
  ...
  PROVIDER_NAME   default monica
  APP_SLUG        default monica
  APP_NAME        default Monica
  EXTERNAL_HOST   default https://monica.white.fm
  INTERNAL_HOST   default http://monica.monica.svc.cluster.local
"""
...
PROVIDER_NAME = os.environ.get("PROVIDER_NAME", "monica")
APP_SLUG = os.environ.get("APP_SLUG", "monica")
APP_NAME = os.environ.get("APP_NAME", "Monica")
EXT = os.environ.get("EXTERNAL_HOST", "https://monica.white.fm")
INT = os.environ.get("INTERNAL_HOST", "http://monica.monica.svc.cluster.local")
...
def get_paged(path: str, params: dict | None = None) -> list:
    ...
    while next_url:
        ...
        nxt_page = pag.get("next", 0)
        if nxt_page:
            sep = "&" if "?" in next_url else "?"
            base_path = next_url.split("?")[0]
            existing_qs = urllib.parse.parse_qs(next_url.split("?")[1]) if "?" in next_url else {}
            existing_qs["page"] = [str(int(nxt_page))]
            next_url = base_path + "?" + urllib.parse.urlencode({k: v[0] for k, v in existing_qs.items()})
        else:
            next_url = ""
    return results
...
def main() -> int:
    ...
    access_token_validity = template_provider.get("access_token_validity", "hours=24")
    refresh_token_validity = template_provider.get("refresh_token_validity", "days=30")
